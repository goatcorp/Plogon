...
class Program
{
    ...
    static async Task Main(
        ...
        DirectoryInfo staticFolder, DirectoryInfo artifactFolder, ModeOfOperation mode,
        FileInfo? buildOverridesFile = null, bool ci = false, bool buildAll = false)
    {
        SetupLogging();
            ...
            else
            {
                ...
                foreach (var task in tasks)
                {
                    ...
                    try
                    {
                        ...
                        GitHubOutputBuilder.StartGroup(
                            $"Build {task.InternalName}[{task.Channel}] ({task.Manifest!.Plugin!.Commit})");
                        ...
                        var status =
                            await buildProcessor.ProcessTask(task, mode == ModeOfOperation.Commit, changelog, tasks);
                        statuses.Add(status);
                        ...
                        if (status.Success)
                        {
                            ...
                            var diffLink = status.DiffUrl == url
                                               ? $"[Repo]({url}) <sup><sup>(New plugin)</sup></sup>"
